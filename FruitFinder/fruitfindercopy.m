...
function [apples, bananas, oranges] = fruitfindercopy(img)
    ...
    %% Apples 
    % Finding locations
    upperRed = imgHSV(:,:,1) <= .99 & imgHSV(:,:,1) >= .8;
    lowerRed = imgHSV(:,:,1) >= 0 & imgHSV(:,:,1) <= .059;
    redSat = imgHSV(:,:,2) >= 0;
    redVal = imgHSV(:,:,3) >= .05 & imgHSV(:,:,3) <= .6;
    appleMask((upperRed | lowerRed) & redSat & redVal) = 1;
    
    % Eroding to get rid of extra noise
    se = strel('disk',2);
    erodeApple = imerode(appleMask, se);
    
    % Closing to get holes closed
    se = strel('disk',7);
    appleClose = imclose(erodeApple, se);
    
    % Opening to get rid of extra extra noise
    se = strel('disk',1);
    appleOpen = imopen(appleClose, se);
    
    se = strel('disk',2);
    appleDilate = imdilate(appleOpen, se);
    
    % imtool(appleMask);
    % imtool(erodeApple);
    % imtool(appleClose);
    % imtool(appleDilate);
    % appleOpen(appleOpen == 0) = 2;
    % appleOpen(appleOpen == 1) = 0;
    % appleOpen(appleOpen == 2) = 1;
    % newMaskCorrect = repmat(appleDilate, [1,1,3]) .* double(img);
    % imgCheckApple = uint8(newMaskCorrect);
    % imtool(rgb2hsv(imgCheckApple));
